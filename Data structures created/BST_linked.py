...
class BST:
    ...
    def _delete_node_left(self, parent):
        ...
        # Traverse the right branch of the parent's left child to find the
        # node with the largest value in the subtree
    ...
    def __eq__(self, target):
        ...
        # your code here
        if self._count == None and  target._count == None:
            equals = True
            
        else:
            equals = self._is_eq_aux(self._root, target._root)
            
        return equals
            
        
    def _is_eq_aux(self, node1, node2):
        """
        Determines if two subs are identical. 
        ...
        returns 
            eq - true if subtree conatins same values
        
        """
        if node1 is None and node2 is None: 
            eq = True 
        elif node1 is not None and node2 is not None and node1._value == node2._value and node1._height == node2._height:
            eq = self._is_eq_aux(node1._left, node2._left) and self._is_eq_aux(node2._right, node2._right)
    ...
    def parent(self, key):
        ...
        while node is not None and has is False:
            if key < node._value:
                parent = node 
                node = node._left 
    ...
    def parent_r(self, key):
        ...
        # your code here
        return self.parent_aux(self._root, key , None )
    
    def parent_aux(self,node,key,parent):
    ...
    def leaf_count_aux(self,node):
        if node is None:
            ...
            counts = self.leaf_count_aux(node._left) + self.leaf_count_aux(node._right)
        return counts
    ...
    def _two_child_count_aux(self, node):
        ...
        if node is None:
            ...
            counts = 1 + self._two_child_count_aux(node._left) + self._two_child_count_aux(node._right)
        else:
            counts = self._two_child_count_aux(node._left) + self._two_child_count_aux(node._right)
        return counts
    ...
    def one_child_count(self):
        ...
        # your code here
        return self.one_child_count_aux(self._root)
    def one_child_count_aux(self, node):
        
        if node is None:
            counts = 0
        elif node._left is None and node._right is not None:
            counts = 1 + self.one_child_count_aux(node._right)
        elif node._left is not None and node._right is None:
            counts = 1 + self.one_child_count_aux(node._left)
        else:
            counts = self._one_child_count_aux(node._left) + self.one_child_count_aux(node._right)
        return counts
    ...
    def node_counts(self):
        ...
        # your code here
        zeros, ones , twos = self.node_counts_aux(self._root)
        return zeros, ones, twos
        
    def node_count_aux(self,node):
        if node is None:
            ...
            zeros, ones, twos = self._node_counts_aux(node._left)
            ones += 1
        elif node._left is None and node._right is not None:
          
            zeros, ones, twos = self._node_counts_aux(node._right)
            ones += 1
        else:
            ...
            zeros = zero_left + zero_right
            ones = one_left + one_right
            twos = two_left + two_right + 1
        return zeros, ones, twos
    ...
    def is_balanced_aux(self, node):
        ...
        if node is None or node._height == 1:
            ...
            balanced = self.is_balanced_aux(node._left) and self.is_balanced_aux(node._right)
    ...
    def is_valid(self):
        ...
        # your code here
        valid = self.is_valid_aux(self._root, None, None)
        return valid
    
    def is_valid_aux(self, node, min_node, max_node):
        """
        """
        if node is None:
            valid = True
            ...
            valid = self.is_valid_aux(node._left, min_node, node) \
                and self.is_valid_aux(node._right, node, max_node)
        return valid
    ...
    def inorder_aux(self, node, a):
        if node is not None:
            ...
            self._norder_aux(node._right, a)
        return
